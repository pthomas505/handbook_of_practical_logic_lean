...
lemma is_small_step_v1_singleton
  ...
  (F : Formula_)
  (Z : String) :
  (is_small_step_v1 [(X, F)] Y Z) ↔ (Y = X ∧ var_occurs_in_formula Z F) :=
  by
  constructor
...
example
  ...
  (h2 : ¬ var_occurs_in_formula X F)
  (h3 : ¬ has_cycle_v1 E) :
  ∃ (Y : String), ∃ (l : List String), var_occurs_in_formula Y F ∧ is_big_step_v1 E Y X l :=
  by
  unfold has_cycle_v1 at h1
  obtain ⟨Y, l, h1⟩ := h1
...
example
  ...
  (h1 : ¬ has_cycle_v1 E)
  (h2 : ¬ var_occurs_in_formula X F)
  (h3 : ∀ (Y : String), ∀ (l : List String), var_occurs_in_formula Y F → ¬ is_big_step_v1 E Y X l) :
  ¬ has_cycle_v1 ((X, F) :: E) :=
  by
  unfold has_cycle_v1 at h1
