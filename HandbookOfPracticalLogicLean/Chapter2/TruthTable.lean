...
/--
  `gen_all_valuations_as_list_of_total_functions init atoms` := A list of all of the possible functions from strings to booleans that map to the same value as `init` for every string not in `atoms`.
-/
