...
/--
  `mk_lits atom_list V` := Returns a formula in conjunction normal form that is only satisfied by valuations that map each atom in `atom_list` to the same boolean value as the valuation `V`.
-/
def mk_lits
